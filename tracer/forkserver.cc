...
static void __tracer_cleanup_trace_bits(void) {
    if (trace_bits) {
        shmdt(trace_bits);
        trace_bits = NULL;
    }
...
static const u8 __trace_count_class_lookup8[256] = {
    [0] = 0,
    [1] = 1,
    [2] = 2,
    [3] = 3,
...
void __trace_init_class_lookup16(void) {
    for (int i = 0; i < 256; i++) {
        for (int j = 0; j < 256; j++) {
            __trace_class_lookup16[(i << 8) | j] = (__trace_count_class_lookup8[i] << 8) | __trace_count_class_lookup8[j];
        }
    }
}
        ...
        if (std::strstr(buf, SUCCESS)) {
            ...
            if (child == 0) {
                ...
                if (found) {
                    ...
                    write(done_trace_fd, SUCCESS, strlen(SUCCESS) + 1);
                    close(done_trace_fd);
                } else {
                    int done_trace_fd = open(DONE_TRACE_PIPE, O_WRONLY);
                    if (done_trace_fd < 0) {
...
__attribute((constructor)) void __tracer_forkserver(int argc, char **argv) {
    ...
    string in_dir, out_dir, out_file_path;
